...
const Notation: React.FC<NotationProps> = ({ notation }) => {
  ...
  notation.split('').forEach((character, index) => {
    ...
    if (
      ...
      character === 'F' ||
      (character === 'B' &&
        allCharacters[index + 1] !== 'l' &&
        allCharacters[index - 1] !== 'M')
    ) {
      platformSpecificNotation.push(
        <Image
          key={index}
          quality={100}
          objectFit="contain"
          src={`/arrows/${directionalMap[character]}.png`}
          width={16}
          height={16}
    ...
    if (
      character === 'X' &&
      allCharacters[index + 1] === 'R' &&
      allCharacters[index + 2] === 'A' &&
      allCharacters[index + 3] === 'Y'
    ) {
      platformSpecificNotation.push(
        <Image
          key={index}
      ...
      platformSpecificNotation.push(<span>+</span>)
      platformSpecificNotation.push(
        <Image
          key={index}
          quality={100}
    ...
    if (character === 'M' && allCharacters[index + 1] === 'B') {
      platformSpecificNotation.push(
        <Image
          key={index}
          quality={100}
          objectFit="contain"
          src={`/${currentNotation}/${currentPlatformMap.MB}.png`}
          width={24}
          height={16}
          alt={`${currentNotation} ${currentPlatformMap.MB}`}
          aria-label={`${currentNotation} ${currentPlatformMap.MB}`}
        />
      )
    }
